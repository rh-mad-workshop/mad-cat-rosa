...
----
NAME                                   DESIRED   CURRENT   READY   AVAILABLE   AGE
rosa-6n4s8-7hbhw-infra-us-east-2a      2         2         2       2           31m
rosa-6n4s8-7hbhw-worker-us-east-2a     2         2         2       2           49m
rosa-6n4s8-7hbhw-workshop-us-east-2a   1         1                             14s
----
+
You can now see the new machineset called named "worker".
...
. Let the above command run until all machines are in the *Running* phase. This means that they are ready and available to run Pods in the cluster. When they are listed as Running you can hit CTRL-C to exit the `oc` command. If you are comfortable with what is happening you can move on to the next steps, which will remove the new nodes.

. But we don't actually need these extra worker nodes so let's scale the cluster back down to a total of 3 worker nodes by scaling down the "Workshop" Machine Pool.
To do so, run the following command:
+
[source,sh,role=execute]
----
